...
import SkeletonLoader from './sliders/SkeletonLoader'; // Импортируем компонент
...
const ImageRestoration: React.FC<ImageRestorationProps> = ({
  ...
  const [loading, setLoading] = useState({ before: true, after: true });
  ...
  const autoplayButton = (
      ...
      <button onClick={toggleAutoplay} className="h-[35px] w-[35px] cursor-pointer p-[0px] rounded-full border-none shadow-xl shadow-black/50" data-testid="autoplay_button">
        {isAutoplaying
            ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="#ffffff" fillRule="evenodd" d="M11.969 2c-5.52 0-10 4.48-10 10s4.48 10 10 10 10-4.48 10-10-4.47-10-10-10Zm-1.25 13.03c0 .48-.2.67-.71.67h-1.3c-.51 0-.71-.19-.71-.67V8.97c0-.48.2-.67.71-.67h1.29c.51 0 .71.19.71.67v6.06h.01Zm5.28 0c0 .48-.2.67-.71.67h-1.29c-.51 0-.71-.19-.71-.67V8.97c0-.48.2-.67.71-.67h1.29c.51 0 .71.19.71.67v6.06Z" /></svg>
            : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="#ffffff" fillRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2Zm3.75 11.299c1-.577 1-2.02 0-2.598l-4.5-2.598A1.5 1.5 0 0 0 9 9.402v5.196a1.5 1.5 0 0 0 2.25 1.3l4.5-2.599Z" clip-rule="evenodd" /></svg>
        }
      </button>
    </span>
  );
  ...
  return (
      ...
      >
        {loading.before && <SkeletonLoader className="absolute h-full object-cover select-none" />}
        <img
            id="beforeImage"
            src={beforeImage}
            className={`absolute h-full object-cover select-none ${loading.before ? 'hidden' : ''}`}
            alt="before"
            onLoad={() => handleImageLoad('before')}
            data-testid="before-image"
        />
        {loading.after && <SkeletonLoader className="absolute h-full object-cover select-none" />}
        <img
            id="afterImage"
            src={afterImage}
            className={`h-full object-cover ${loading.after ? 'hidden' : ''}`}
            alt="after"
            style={{ userSelect: "none", clipPath: `inset(0 0 0 ${sliderX})`, pointerEvents: "auto" }}
