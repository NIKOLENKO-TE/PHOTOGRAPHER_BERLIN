...
const SetLanguage: React.FC<SetLanguageProps> = ({ onSelectLanguage }): JSX.Element => {
  const locales = {
          ...
          <div className="relative inline-block text-left">
            <span className="rounded-md shadow-sm">
              <Listbox.Button className="inline-flex justify-center ssm:h-[38px] md:h-[40px] xl:h-[42px] w-full rounded-[12px] ssm:text-[16px]  md:text-[18px] xl:text-[20px] text-white whitespace-nowrap place-content-stretch bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br border-blue-600 shadow-lg shadow-blue-500/50 mr-1">
                {languageInfo && <img src={languageInfo.flag} alt={languageInfo.title} className="w-[30px] h-[20px] ssm:mt-1 md:mt-1.5 mr-1 object-cover" />}
                {languageInfo?.title}
              </Listbox.Button>
            </span>

            <Transition as={React.Fragment} leave="transition ease-in duration-200" leaveFrom="opacity-100" leaveTo="opacity-0">
              <Listbox.Options
                static
                ...
                {Object.keys(locales).map((locale) => (
                  <Listbox.Option key={locale} value={locale}>
                    {({ selected }) => (
                      <div
                        className={`${
                          selected ? 'bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br border-blue-600 shadow-lg shadow-blue-500/50 text-white' : 'text-black'
                      ...
                      >
                        <img src={locales[locale as keyof typeof locales].flag} alt={locales[locale as keyof typeof locales].title} className="w-[35px] mr-2 shadow-md shadow-black/20" />
